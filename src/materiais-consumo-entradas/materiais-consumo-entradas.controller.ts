...
export class MateriaisConsumoEntradasController {
    constructor(private materialConsumoEntradasService: MateriaisConsumoEntradasService){}

    @Get()
    async index(@Request() req):Promise<MateriaisConsumoEntradas>{
        return this.materialConsumoEntradasService.index(req.user);
    }

    @Get(':id')
    async find(@Param('id') id: number, @Request() req):Promise<MaterialConsumoEntrada>  {
    ...
    @Put(':id')
    async update(@Param('id') id: number, @Body() object: MaterialConsumoEntrada, @Request() req) {
