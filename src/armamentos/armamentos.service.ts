...
import {
  Armamento as ArmamentoInterface,
  Armamentos as ArmamentosInterface,
} from './armamento.interface';
...
export class ArmamentosService {
  ...
  async create(object: ArmamentoInterface, idUser: User) {
    const object2: ArmamentoInterface = this.armamentoRepository.create({
      ...object,
      quantidade_disponivel: object.quantidade,
      created_by: idUser,
    });
    const save = await this.armamentoRepository.save(object2);
  ...
  async update(id: number, object: ArmamentoInterface, idUser: User) {
    let data: ArmamentoInterface = await this.armamentoRepository.findOneBy({
      id: id,
    });
    data = { ...object };
    await this.armamentoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    const data = await this.armamentoRepository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async atualizarQuantidadeUp(id: number, quantidade: number): Promise<void> {
    const data: ArmamentoInterface = await this.armamentoRepository.findOneBy({
      id: id,
    });
  ...
  async ajustarquant(id: number, object: any, idUser: User) {
    ...
    if (object.tipo == 1) {
      ...
      data.quantidade_disponivel =
        Number(data.quantidade_disponivel) + Number(object.quantidade);
    } else {
      data.quantidade = Number(data.quantidade) - Number(object.quantidade);
      data.quantidade_disponivel =
        Number(data.quantidade_disponivel) - Number(object.quantidade);
  ...
  async quantPorModelo(
    params: any,
    idUser: User,
  ): Promise<ArmamentosInterface> {
    return await this.armamentoRepository
      .createQueryBuilder('armamentos')
      ...
      .where('armamentos.subunidadeId = :subunidadeId', {
        subunidadeId: params.subunidade,
      })
