...
import {
  MaterialPolicial as MaterialPolicialInterface,
  MateriaisPoliciais as MateriaisPoliciaisInterface,
} from './material-policial.interface';
...
export class MateriaisPoliciaisService {
  ...
  async find(
    id: number,
    idUser: User,
  ): Promise<MaterialPolicialInterface | null> {
    var idsSubs: any = [];
  ...
  async create(object: MaterialPolicialInterface, idUser: User) {
    var object2: MaterialPolicialInterface =
      this.materiaisPoliciaisRepository.create({
        ...object,
  ...
  async update(id: number, object: MaterialPolicialInterface, idUser: User) {
    var data: MaterialPolicialInterface =
      await this.materiaisPoliciaisRepository.findOneBy({ id: id });
    data = { ...object };
    await this.materiaisPoliciaisRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    ...
    var data = await this.materiaisPoliciaisRepository.findOne({
      where: {
        id: id,
      },
    });
    ...
    mats.forEach((arm) => {
      this.materiaisService.atualizarQuantidadeUp(
        arm.material.id,
        arm.quantidade,
      );
    });
  ...
  async receber(object, idUser: User) {
    var data: MaterialPolicialInterface =
      await this.materiaisPoliciaisRepository.findOneBy({ id: object.id });
    ...
    object.materiais.forEach(async (element) => {
      var armemp: MaterialPolicialItem =
        await this.materiaisPoliciaisIntensService.find(element.id);
      if (element.quantidade != armemp.quantidade) {
        await this.materiaisService.atualizarQuantidadeUp(
          armemp.material.id,
          element.quantidade,
        );
        armemp.quantidade_devolucao = element.quantidade;
        await this.materiaisPoliciaisIntensService.update(
          armemp.id,
          armemp,
          idUser,
        );
        var dif = armemp.quantidade - element.quantidade;
        var arma: Material = await this.materiaisService.find2(
          armemp.material.id,
          idUser,
        );
  ...
  async emprestados(
    params: any,
    idUser: User,
  ): Promise<MateriaisPoliciaisInterface> {
