...
export class MateriaisConsumoSaidasController {
    constructor(private materialConsumoSaidaService: MateriaisConsumoSaidasService){}

    @Get()
    async index(@Request() req):Promise<MateriaisConsumoSaidas>{
        return this.materialConsumoSaidaService.index(req.user);
    }

    @Get(':id')
    async find(@Param('id') id: number, @Request() req):Promise<MaterialConsumoSaida>  {
    ...
    @Put(':id')
    async update(@Param('id') id: number, @Body() object: MaterialConsumoSaida, @Request() req) {
