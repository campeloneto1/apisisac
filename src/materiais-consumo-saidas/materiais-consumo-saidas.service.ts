...
import {
  MaterialConsumoSaida as MaterialConsumoSaidaInterface,
  MateriaisConsumoSaidas as MateriaisConsumoSaidasInterface,
...
export class MateriaisConsumoSaidasService {
  ...
  async index(
    params: any,
    idUser: User,
  ): Promise<MateriaisConsumoSaidasInterface> {
  ...
  async find(
    id: number,
    idUser: User,
  ): Promise<MaterialConsumoSaidaInterface | null> {
    var idsSubs: any = [];
  ...
  async create(object: MaterialConsumoSaidaInterface, idUser: User) {
    var object2: MaterialConsumoSaidaInterface =
      this.amaterialConsumoSaidaRepository.create({
        ...object,
        data_saida: new Date(),
        created_by: idUser,
      });
  ...
  async update(
    id: number,
    object: MaterialConsumoSaidaInterface,
    idUser: User,
  ) {
    var data: MaterialConsumoSaidaInterface =
      await this.amaterialConsumoSaidaRepository.findOneBy({ id: id });
    data = { ...object };
    await this.amaterialConsumoSaidaRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    ...
    var data = await this.amaterialConsumoSaidaRepository.findOne({
      where: {
        id: id,
      },
    });
