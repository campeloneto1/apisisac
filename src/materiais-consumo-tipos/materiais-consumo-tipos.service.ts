...
import {
  MaterialConsumoTipo as MaterialConsumoTipoInterface,
  MateriaisConsumoTipos as MateriaisConsumoTiposInterface,
...
export class MateriaisConsumoTiposService {
  ...
  async find(id: number): Promise<MaterialConsumoTipoInterface | null> {
    return await this.materialConsumoTipoRepository.findOne({
      where: { id: id },
    });
  }

  async create(object: MaterialConsumoTipoInterface, idUser: User) {
    var object: MaterialConsumoTipoInterface =
      this.materialConsumoTipoRepository.create({
        ...object,
        created_by: idUser,
      });
  ...
  async update(id: number, object: MaterialConsumoTipoInterface, idUser: User) {
    var data: MaterialConsumoTipoInterface =
      await this.materialConsumoTipoRepository.findOneBy({ id: id });
    data = { ...object };
    await this.materialConsumoTipoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.materialConsumoTipoRepository.findOne({
      where: {
        id: id,
      },
    });
