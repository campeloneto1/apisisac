...
import { DocumentoTipo as DocumentoTipoInterface, DocumentosTipos as DocumentosTiposInterface } from './documento-tipo.interface';
...
export class DocumentosTiposService {
    ...
    async index(): Promise<DocumentosTiposInterface> {
      ...
      async create(object: DocumentoTipoInterface, idUser: User) {
        var object:DocumentoTipoInterface = this.documentoTipoRepository.create({...object, created_by: idUser}) 
      ...
      async update(id:number, object: DocumentoTipoInterface, idUser: User) {
        var data: DocumentoTipoInterface = await this.documentoTipoRepository.findOneBy({id: id});
        data = {...object}
        await this.documentoTipoRepository.update({id:id},{...data, updated_by: idUser});
        await this.logsService.create({
      ...
      async remove(id: number, idUser: User) {
        var data = await this.documentoTipoRepository.findOne({where: {
          id: id,
        }});
