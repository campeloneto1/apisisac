...
import {
  VeiculoOficina as VeiculoOficinaInterface,
  VeiculosOficinas as VeiculosOficinasInterface,
} from './veiculo-oficina.interface';
...
export class VeiculosOficinasService {
  ...
  async find(
    id: number,
    idUser: User,
  ): Promise<VeiculoOficinaInterface | null> {
    var idsSubs: any = [];
    idUser.users_subunidades.forEach((data) => {
      idsSubs.push(data.subunidade.id);
    });
    return await this.veiculoOficinaRository.findOne({
      where: {
        id: id,
        //@ts-ignore
        subunidade: {
  ...
  async update(id: number, object: VeiculoOficinaInterface, idUser: User) {
    ...
    var data: VeiculoOficinaInterface =
      await this.veiculoOficinaRository.findOneBy({ id: id });
    data = { ...object };
    await this.veiculoOficinaRository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.veiculoOficinaRository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async receber(object: any, idUser: User) {
    var data: VeiculoOficinaInterface =
      await this.veiculoOficinaRository.findOneBy({ id: object.id });
    data = {
      ...data,
      data_final: new Date(),
  ...
  async emmanutencao(
    params: any,
    idUser: User,
  ): Promise<VeiculosOficinasInterface> {
    return await this.veiculoOficinaRository.find({
      where: {
        data_final: IsNull(),
        //@ts-ignore
