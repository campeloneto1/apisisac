...
import { Documento as DocumentoInterface, Documentos as DocumentosInterface } from './documento.interface';
...
export class DocumentosService {
    ...
    async index(): Promise<DocumentosInterface> {
      ...
      async create(object: DocumentoInterface, idUser: User) {
        ...
        var object:DocumentoInterface = this.documentoRepository.create({...object, codigo: cod, setor: idUser.policial.setor, created_by: idUser});
      ...
      async update(id:number, object: DocumentoInterface, idUser: User) {
        var data: DocumentoInterface = await this.documentoRepository.findOneBy({id: id});
        ...
        await this.documentoRepository.update({id:id},{...data, updated_by: idUser});
      ...
      async remove(id: number, idUser: User) {
        var data = await this.documentoRepository.findOne({where: {
          id: id,
        }});
