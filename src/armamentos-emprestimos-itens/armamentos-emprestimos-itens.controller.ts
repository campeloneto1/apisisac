...
export class ArmamentosEmprestimosItensController {
  ...
  async create(@Body() object: ArmamentoEmprestimoItem, @Request() req) {
    return await this.armamentosEmprestimosItensService.create(
      object,
      req.user,
    );
  }
  ...
  async update(
    @Param('id') id: number,
    @Body() object: ArmamentoEmprestimoItem,
    @Request() req,
  ) {
    return await this.armamentosEmprestimosItensService.update(
      id,
      object,
      req.user,
    );
