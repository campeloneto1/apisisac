...
import {
  AfastamentoTipo as AfastamentoTipoInterface,
  AfastamentosTipos as AfastamentosTiposInterface,
} from './afastamento-tipo.interface';
...
export class AfastamentosTiposService {
  ...
  async create(object: AfastamentoTipoInterface, idUser: User) {
    var object: AfastamentoTipoInterface =
      this.afastamentoTipoRepository.create({ ...object, created_by: idUser });
  ...
  async update(id: number, object: AfastamentoTipoInterface, idUser: User) {
    var data: AfastamentoTipoInterface =
      await this.afastamentoTipoRepository.findOneBy({ id: id });
    data = { ...object };
    await this.afastamentoTipoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.afastamentoTipoRepository.findOne({
      where: {
        id: id,
      },
    });
