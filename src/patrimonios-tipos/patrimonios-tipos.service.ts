...
import {
  PatrimonioTipo as PatrimonioTipoInterface,
  PatrimoniosTipos as PatrimoniosTiposInterface,
} from './patrimonio-tipo.interface';
...
export class PatrimoniosTiposService {
  ...
  async create(object: PatrimonioTipoInterface, idUser: User) {
    var object: PatrimonioTipoInterface = this.patrimonioTipoRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: PatrimonioTipoInterface, idUser: User) {
    var data: PatrimonioTipoInterface =
      await this.patrimonioTipoRepository.findOneBy({ id: id });
    data = { ...object };
    await this.patrimonioTipoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.patrimonioTipoRepository.findOne({
      where: {
        id: id,
      },
    });
