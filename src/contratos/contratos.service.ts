...
import {
  Contrato as ContratoInterface,
  Contratos as ContratosInterface,
...
export class ContratosService {
  ...
  async create(object: ContratoInterface, idUser: User) {
    var object: ContratoInterface = this.contratoRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: ContratoInterface, idUser: User) {
    var data: ContratoInterface = await this.contratoRepository.findOneBy({
      id: id,
    });
  ...
  async remove(id: number, idUser: User) {
    var data = await this.contratoRepository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async aditivar(id: number, object: any, idUser: User) {
    ...
    data.valor_global =
      Number(data.valor_global) +
      Number(data.valor_global) * (Number(object.porcentagem_aditivado) / 100);
    if (data.quantidade_diarias) {
      data.quantidade_diarias =
        Number(data.quantidade_diarias) +
        Number(data.quantidade_diarias) *
          (Number(object.porcentagem_aditivado) / 100);
    }
