...
import {
  Pais as PaisInterface,
  Paises as PaisesInterface,
} from './pais.interface';
...
export class PaisesService {
  ...
  async create(object: PaisInterface, idUser: User) {
    var object: PaisInterface = this.paisRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: PaisInterface, idUser: User) {
    ...
    data = { ...object };
    await this.paisRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.paisRepository.findOne({
      where: {
        id: id,
      },
    });
