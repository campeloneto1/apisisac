...
import {
  ContratoObjeto as ContratoObjetoInterface,
  ContratosObjetos as ContratosObjetosInterface,
} from './contrato-objeto.interface';
...
export class ContratosObjetosService {
  ...
  async create(object: ContratoObjetoInterface, idUser: User) {
    var object: ContratoObjetoInterface = this.corRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: ContratoObjetoInterface, idUser: User) {
    var data: ContratoObjetoInterface = await this.corRepository.findOneBy({
      id: id,
    });
    data = { ...object };
    await this.corRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.corRepository.findOne({
      where: {
        id: id,
      },
    });
