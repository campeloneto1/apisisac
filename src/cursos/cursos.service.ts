...
import {
  Curso as CursoInterface,
  Cursos as CursosInterface,
} from './curso.interface';
...
export class CursosService {
  ...
  async create(object: CursoInterface, idUser: User) {
    var object: CursoInterface = this.cursoRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: CursoInterface, idUser: User) {
    ...
    data = { ...object };
    await this.cursoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.cursoRepository.findOne({
      where: {
        id: id,
      },
    });
