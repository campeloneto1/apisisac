...
import {
  Modelo as ModeloInterface,
  Modelos as ModelosInterface,
} from './modelo.interface';
...
export class ModelosService {
  ...
  async create(object: ModeloInterface, idUser: User) {
    var object: ModeloInterface = this.modeloRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: ModeloInterface, idUser: User) {
    var data: ModeloInterface = await this.modeloRepository.findOneBy({
      id: id,
    });
    data = { ...object };
    await this.modeloRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.modeloRepository.findOne({
      where: {
        id: id,
      },
    });
