...
export class OficinasService {
    ...
    async index(idUser: User): Promise<OficinasInterface> {
      ...
      async find(id: number, idUser: User): Promise<OficinaInterface | null> {
        return await this.oficinaRepository.findOne({where: {
          id: id,
          //@ts-ignore
          subunidade: {
            id: idUser.subunidade.id
      ...
      async create(object: OficinaInterface, idUser: User) {
        var object:OficinaInterface = this.oficinaRepository.create({...object, subunidade: idUser.subunidade, created_by: idUser}) 
        await this.oficinaRepository.save(object);      
      }
  
      async update(id:number, object: OficinaInterface, idUser: User) {
        var data: OficinaInterface = await this.oficinaRepository.findOneBy({id: id});
        data = {...object}
