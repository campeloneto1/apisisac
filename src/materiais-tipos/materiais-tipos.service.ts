...
import {
  MaterialTipo as MaterialTipoInterface,
  MateriaisTipos as MateriaisTiposInterface,
} from './material-tipo.interface';
...
export class MateriaisTiposService {
  ...
  async create(object: MaterialTipoInterface, idUser: User) {
    var object: MaterialTipoInterface = this.materialTipoRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: MaterialTipoInterface, idUser: User) {
    var data: MaterialTipoInterface =
      await this.materialTipoRepository.findOneBy({ id: id });
    data = { ...object };
    await this.materialTipoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.materialTipoRepository.findOne({
      where: {
        id: id,
      },
    });
