...
import {
  MaterialConsumoSaidaItem as MaterialConsumoSaidaItemInterface,
  MateriaisConsumoSaidasItens as MateriaisConsumoSaidasItensInterface,
...
export class MateriaisConsumoSaidasItensService {
  ...
  async find(id: number): Promise<MaterialConsumoSaidaItemInterface | null> {
    return await this.materialConsumoSaidaItemRepository.findOne({
      where: { id: id },
    });
  }

  async create(objectreq: MaterialConsumoSaidaItemInterface, idUser: User) {
    const object: MaterialConsumoSaidaItemInterface =
      this.materialConsumoSaidaItemRepository.create({
        ...objectreq,
        created_by: idUser,
      });
    const save = await this.materialConsumoSaidaItemRepository.save(object);
  ...
  async update(
    id: number,
    object: MaterialConsumoSaidaItemInterface,
    idUser: User,
  ) {
    let data: MaterialConsumoSaidaItemInterface =
      await this.materialConsumoSaidaItemRepository.findOneBy({ id: id });
    data = { ...object };
    await this.materialConsumoSaidaItemRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    const object: MaterialConsumoSaidaItemInterface =
      await this.materialConsumoSaidaItemRepository.findOne({
        where: { id: id },
      });
