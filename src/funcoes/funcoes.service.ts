...
import { Funcao as FuncaoInterface, Funcoes as FuncoesInterface } from './funcao.interface';
@Injectable()
export class FuncoesService {
    ...
    async index(): Promise<FuncoesInterface> {
      ...
      async create(object: FuncaoInterface, idUser: User) {
        var object:FuncaoInterface = this.funcaoRepository.create({...object, created_by: idUser}) 
      ...
      async update(id:number, object: FuncaoInterface, idUser: User) {
        var data: FuncaoInterface = await this.funcaoRepository.findOneBy({id: id});
        data = {...object}
        await this.funcaoRepository.update({id:id},{...data, updated_by: idUser});
        await this.logsService.create({
      ...
      async remove(id: number, idUser: User) {
        var data = await this.funcaoRepository.findOne({where: {
          id: id,
        }});
