...
export class UsersService {
  ...
  async create(object: UserInterface, idUser: UserInterface) {
    ...
    object.password = await this.utilitiesService.hashString(
      `${object.cpf}${object.salt}`,
    );

    var object: UserInterface = this.usersRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async remove(id: number, idUser: UserInterface) {
    var data = await this.usersRepository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async resetPass(object: UserInterface, idUser: UserInterface) {
    ...
    user.password = await this.utilitiesService.hashString(
      `${user.cpf}${user.salt}`,
    );
    var update = await this.usersRepository.update(
      { id: object.id },
      { ...user },
    );
    await this.logsService.create({
