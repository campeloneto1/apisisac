...
import {
  MaterialPolicialItem as MaterialPolicialItemInterface,
  MateriaisPoliciaisItens as MateriaisPoliciaisItensInterface,
} from './material-policial-item.interface';
...
export class MateriaisPoliciaisItensService {
  ...
  async find(id: number): Promise<MaterialPolicialItemInterface | null> {
    return await this.materialPolicialItemRepository.findOne({
      where: { id: id },
    });
  }

  async create(objectreq: MaterialPolicialItemInterface, idUser: User) {
    const object: MaterialPolicialItemInterface =
      this.materialPolicialItemRepository.create({
        ...objectreq,
        created_by: idUser,
      });
    const save = await this.materialPolicialItemRepository.save(object);

    this.materiaisService.atualizarQuantidadeDown(
      //@ts-ignore
      object.material,
      object.quantidade,
    );
  ...
  async update(
    id: number,
    object: MaterialPolicialItemInterface,
    idUser: User,
  ) {
    let data: MaterialPolicialItemInterface =
      await this.materialPolicialItemRepository.findOneBy({ id: id });
    data = { ...object };
    await this.materialPolicialItemRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    const object: MaterialPolicialItemInterface =
      await this.materialPolicialItemRepository.findOne({ where: { id: id } });
