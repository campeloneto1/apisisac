...
import {
  Unidade as UnidadeInterface,
  Unidades as UnidadesInterface,
...
export class UnidadesService {
  ...
  async create(object: UnidadeInterface, idUser: User) {
    var object: UnidadeInterface = this.unidadeRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: UnidadeInterface, idUser: User) {
    var data: UnidadeInterface = await this.unidadeRepository.findOneBy({
      id: id,
    });
  ...
  async remove(id: number, idUser: User) {
    var data = await this.unidadeRepository.findOne({
      where: {
        id: id,
      },
    });
