...
import {
  ArmamentoEmprestimo as ArmamentoEmprestimoInterface,
  ArmamentosEmprestimos as ArmamentosEmprestimosInterface,
...
export class ArmamentosEmprestimosService {
  ...
  async index(
    params: any,
    idUser: User,
  ): Promise<ArmamentosEmprestimosInterface> {
  ...
  async find(
    id: number,
    idUser: User,
  ): Promise<ArmamentoEmprestimoInterface | null> {
    var idsSubs: any = [];
  ...
  async create(object: ArmamentoEmprestimoInterface, idUser: User) {
    var object2: ArmamentoEmprestimoInterface =
      this.armamentoEmprestimoRepository.create({
        ...object,
  ...
  async update(id: number, object: ArmamentoEmprestimoInterface, idUser: User) {
    var data: ArmamentoEmprestimoInterface =
      await this.armamentoEmprestimoRepository.findOneBy({ id: id });
    data = { ...object };
    await this.armamentoEmprestimoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    ...
    var data = await this.armamentoEmprestimoRepository.findOne({
      where: {
        id: id,
      },
    });
    ...
    arms.forEach((arm) => {
      this.armamentoService.atualizarQuantidadeUp(
        arm.armamento.id,
        arm.quantidade,
      );
    });
  ...
  async receber(object, idUser: User) {
    var data: ArmamentoEmprestimoInterface =
      await this.armamentoEmprestimoRepository.findOneBy({ id: object.id });
    data.observacoes = object.observacoes;
    await this.armamentoEmprestimoRepository.update(
      { id: object.id },
      { ...data, data_devolucao: new Date(), updated_by: idUser },
    );

    object.armamentos.forEach(async (element) => {
      var armemp: ArmamentoEmprestimoItem =
        await this.armamentosEmprestimosItensService.find(element.id);
      if (element.quantidade != armemp.quantidade) {
        await this.armamentoService.atualizarQuantidadeUp(
          armemp.armamento.id,
          element.quantidade,
        );
        armemp.quantidade_devolucao = element.quantidade;
        await this.armamentosEmprestimosItensService.update(
          armemp.id,
          armemp,
          idUser,
        );
        var dif = armemp.quantidade - element.quantidade;
        var arma: Armamento = await this.armamentoService.find2(
          armemp.armamento.id,
          idUser,
        );
