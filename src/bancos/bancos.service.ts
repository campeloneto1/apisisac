...
import { Banco as BancoInterface, Bancos as BancosInterface } from './banco.interface';
...
export class BancosService {
    ...
    async index(): Promise<BancosInterface> {
      ...
      async create(object: BancoInterface, idUser: User) {
        var object:BancoInterface = this.bancoRepository.create({...object, created_by: idUser}) 
      ...
      async update(id:number, object: BancoInterface, idUser: User) {
        ...
        data = {...object}
        await this.bancoRepository.update({id:id},{...data, updated_by: idUser});
        await this.logsService.create({
      ...
      async remove(id: number, idUser: User) {
        var data = await this.bancoRepository.findOne({where: {
          id: id,
        }});
