...
export class PoliciaisCursosController {
    ...
    @Get()
    async index(@Request() req):Promise<PoliciaisCursos>{
        return this.policiaisCursosService.index(req.user);
    }
    ...
    @Put(':id')
    async update(@Param('id') id: number, @Body() object: PolicialCurso, @Request() req) {
        return await this.policiaisCursosService.update(id, object, req.user);
    }

    @Delete(':id')
    async remove(@Param('id') id: number, @Request() req) {
        return await this.policiaisCursosService.remove(id, req);
    }
}
