...
import {
  Subunidade as SubunidadeInterface,
  Subunidades as SubunidadesInterface,
} from './subunidade.interface';
...
export class SubunidadesService {
  ...
  async find(id: number): Promise<SubunidadeInterface | null> {
    return await this.subunidadeRepository.findOne({
      where: { id: id },
      relations: {
        comandante: {
          setor: false,
  ...
  async create(object: SubunidadeInterface, idUser: User) {
    var object: SubunidadeInterface = this.subunidadeRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: SubunidadeInterface, idUser: User) {
    var data: SubunidadeInterface = await this.subunidadeRepository.findOneBy({
      id: id,
    });
    data = { ...object };
    await this.subunidadeRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.subunidadeRepository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async whereUnidade(
    id: number,
    idUser: User,
  ): Promise<SubunidadesInterface | null> {
    var idsSubs: any = [];
