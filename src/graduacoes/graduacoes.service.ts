...
import {
  Graduacao as GraduacaoInterface,
  Graduacoes as GraduacoesInterface,
} from './graduacao.interface';
...
export class GraduacoesService {
  ...
  async create(object: GraduacaoInterface, idUser: User) {
    var object: GraduacaoInterface = this.graduacaoRepository.create({
      ...object,
      created_by: idUser,
    });
  ...
  async update(id: number, object: GraduacaoInterface, idUser: User) {
    var data: GraduacaoInterface = await this.graduacaoRepository.findOneBy({
      id: id,
    });
    data = { ...object };
    await this.graduacaoRepository.update(
      { id: id },
  ...
  async remove(id: number, idUser: User) {
    var data = await this.graduacaoRepository.findOne({
      where: {
        id: id,
      },
    });
  ...
  async policiaisGraduacoes(params: any, idUser: User): Promise<any> {
    return await this.graduacaoRepository.query(`
